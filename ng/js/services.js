    ...
    .factory('i18n', ['$q', '$http', function ($q, $http) {
        ...
        var get_translations = function () {
            ...
            if (!translations) {
                ...
                var url =
                    ajaxurl +
                    "?action=wpcfs_ng_load_translations" +
                    "&nonce=" +
                    encodeURIComponent(wpcfsAdmin.adminNonce);

                translations = $http.get(url);
            }
    ...
    .factory("taxonomyLister", ['$http', '$q', function ($http) {
        ...
        return function (taxonomy) {
            if (typeof loaded[taxonomy] == 'undefined') {
                var url =
                    ajaxurl +
                    "?action=wpcfs_ng_load_taxonomy" +
                    "&taxonomy=" + encodeURIComponent(taxonomy) +
                    "&nonce=" + encodeURIComponent(wpcfsAdmin.adminNonce);
                loaded[taxonomy] =
                    $http.get(url)
